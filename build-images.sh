...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-grafana; then
    ...
    buildah from --name nodebuilder-grafana -v "${PWD}:/usr/src:Z" docker.io/library/node:24.14.1-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/grafana/grafana:13.0.1" \
    "${container}"
# Commit the image
