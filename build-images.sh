...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-grafana; then
    ...
    buildah from --name nodebuilder-grafana -v "${PWD}:/usr/src:Z" docker.io/library/node:18.20.8-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/grafana/grafana:12.3.0" \
    "${container}"
# Commit the image
